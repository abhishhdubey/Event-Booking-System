...
">
    ...
    <?php if ($hasBanner): ?>
        <div
            style="position:absolute;inset:0;background:linear-gradient(to bottom, rgba(0,0,0,0.35) 0%, rgba(0,0,0,0.8) 100%);">
        </div>
        <div
            style="position:absolute;inset:0;background:linear-gradient(to right, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.70) 50%, rgba(0,0,0,0.20) 100%);">
        </div>
    <?php endif; ?>
    ...
    <div class="container" style="position:relative;z-index:2;padding:50px 15px;width:100%;">
        <div style="display:grid;grid-template-columns:220px 1fr;gap:40px;align-items:start;">
            ...
            <div style="position:relative;">
                ...
                ?>
                <div
                    style="aspect-ratio:2/3;background:linear-gradient(135deg,var(--bg-card),var(--border));border-radius:var(--radius);overflow:hidden;border:1px solid rgba(255,255,255,0.1);box-shadow:0 8px 40px rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;font-size:5rem;position:relative;">
                    <?php if ($hasPoster): ?>
                        <img src="<?php echo $posterSrc; ?>" style="width:100%;height:100%;object-fit:cover;display:block;"
                ...
                <div class="movie-rating-badge"
                    style="position:absolute;top:10px;left:10px;font-size:0.9rem;padding:5px 12px;transform:none;">⭐
            ...
            <div>
                <h1
                    style="font-size:2.8rem;font-weight:800;margin-bottom:12px;text-shadow:0 2px 12px rgba(0,0,0,0.8);line-height:1.2;">
                    <?php echo htmlspecialchars($movie['title']); ?>
                </h1>
                <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:16px;">
                    <?php foreach (explode(',', $movie['genre']) as $g): ?>
                        <span class="badge badge-info"
                            style="backdrop-filter:blur(6px);"><?php echo htmlspecialchars(trim($g)); ?></span>
                    <?php endforeach; ?>
                </div>
                <div
                    style="display:flex;gap:24px;flex-wrap:wrap;margin-bottom:20px;color:rgba(255,255,255,0.85);font-size:0.95rem;">
                    <span style="text-shadow:0 1px 4px rgba(0,0,0,0.8);">🌐
                        <?php echo htmlspecialchars($movie['language']); ?></span>
                    <span style="text-shadow:0 1px 4px rgba(0,0,0,0.8);">🕐
                        <?php echo htmlspecialchars($movie['duration']); ?></span>
                    <span style="text-shadow:0 1px 4px rgba(0,0,0,0.8);">📅
                        <?php echo date('d M Y', strtotime($movie['release_date'])); ?></span>
                </div>
                <p
                    style="color:rgba(255,255,255,0.78);line-height:1.8;margin-bottom:24px;max-width:600px;text-shadow:0 1px 6px rgba(0,0,0,0.7);">
                    <?php echo htmlspecialchars($movie['description']); ?>
                </p>
                <div style="display:flex;gap:12px;flex-wrap:wrap;">
                    <a href="theatres.php?movie_id=<?php echo $movie['movie_id']; ?>"
                        class="btn btn-primary btn-lg pulse">🎟️ Book Tickets</a>
                    <?php if ($movie['trailer_url']): ?>
                        <button class="btn btn-dark btn-lg"
                            onclick="openTrailer('<?php echo str_replace("'", "\\'", $movie['trailer_url']); ?>')"
                            style="background:rgba(0,0,0,0.5);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,0.2);">▶
                    ...
                    <?php if (isset($_SESSION['user_id'])): ?>
                        <form method="POST" action="wishlist.php">
                            ...
                            <button type="submit" class="btn btn-outline btn-lg"
                                style="background:rgba(0,0,0,0.5);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,0.2);"><?php echo $inWishlist ? '💔 Remove Wishlist' : '❤️ Add to Wishlist'; ?></button>
                        </form>
                    <?php endif; ?>
                </div>
            </div>
...
<div class="container" style="padding:40px 15px;">
    ...
    <div id="tab-overview" class="tab-content">
        <div style="display:grid;grid-template-columns:2fr 1fr;gap:40px;">
            ...
            <div
                style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:24px;height:fit-content;">
                ...
                foreach ($details as $key => $val):
                    ?>
                    <div
                        style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);font-size:0.9rem;">
    ...
    <div id="tab-showtimes" class="tab-content" style="display:none;">
        ...
        if (!$allShows || $allShows->num_rows === 0): ?>
            <p style="color:var(--text-muted);padding:30px;text-align:center;">No upcoming shows
                available.<?php echo $selectedCity ? ' Try removing the city filter.' : ''; ?></p>
        <?php else:
            // Group shows by date
            $byDate = [];
            ...
            $first = true;
            foreach ($byDate as $date => $theatresOnDate): ?>
                <div class="date-pane date-pane-<?php echo str_replace('-', '', $date); ?>"
                    style="<?php echo $first ? 'display:block' : 'display:none'; ?>">
                            ...
                            <div class="theatre-header">
                                <div>
                                    ...
                                    <div class="theatre-location">📍
                                        <?php echo htmlspecialchars($info['city'] . ' - ' . $info['location']); ?>
                                        <?php if (!empty($info['map_link'])): ?> &nbsp;<a
                                                href="<?php echo htmlspecialchars($info['map_link']); ?>" target="_blank"
                                                style="font-size:.8rem;color:var(--primary);">🗺️ Map</a><?php endif; ?>
                                    </div>
                                </div>
                            </div>
                            <div class="show-times" style="flex-wrap:wrap;gap:12px;">
                                <?php foreach ($tshows as $show):
                                    ...
                                    <a href="seat-selection.php?show_id=<?php echo $show['show_id']; ?>" class="show-time-btn"
                                        style="display:flex;flex-direction:column;align-items:center;gap:4px;padding:10px 16px;min-width:100px;">
                                        <span
                                            style="background:<?php echo $langColor; ?>22;color:<?php echo $langColor; ?>;padding:2px 8px;border-radius:4px;font-size:.68rem;font-weight:700;letter-spacing:.5px;"><?php echo htmlspecialchars(strtoupper($lang)); ?></span>
                                        <span
                                            style="font-weight:700;font-size:.95rem;"><?php echo date('h:i A', strtotime($show['show_time'])); ?></span>
                                        <small
                                            style="color:var(--text-muted);font-size:.72rem;">₹<?php echo number_format($show['price'], 0); ?></small>
                                    </a>
                                <?php endforeach; ?>
                            </div>
                        </div>
    ...
    <div id="tab-reviews" class="tab-content" style="display:none;">
        ...
        <?php if (isset($_SESSION['user_id'])): ?>
            <div
                style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:24px;margin-bottom:30px;">
                ...
                <form method="POST">
                    <div class="form-group">
                        ...
                        <div style="display:flex;gap:6px;margin-bottom:10px;" id="starRating">
                            <?php for ($i = 1; $i <= 10; $i++): ?>
                                <span class="star-label" style="font-size:1.5rem;cursor:pointer;color:var(--border);"
        ...
        <?php else: ?>
            <div class="alert alert-warning">⚠️ <a href="login.php" style="color:var(--primary);">Login</a> to write a
                review.</div>
        <?php endif; ?>
                ...
                <div
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                        ...
                        <div style="display:flex;gap:4px;align-items:center;">
                            ...
                            <span
                                style="color:var(--text-muted);font-size:0.8rem;margin-left:10px;"><?php echo date('d M Y', strtotime($rev['review_date'])); ?></span>
                        </div>
                    </div>
