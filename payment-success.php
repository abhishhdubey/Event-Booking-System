...
$booking = $conn->query("
    SELECT b.*, m.title, m.language, t.theatre_name, t.city, s.show_date, s.show_time, s.price,
           p.payment_method, p.payment_status, u.name as user_name, u.email
    FROM bookings b
...
$pageTitle = 'Booking Confirmed! - BookYourShow';
...
<div class="success-wrapper">
    <div class="success-card" id="printArea">
        ...
        <p style="color:var(--text-muted);margin-bottom:4px;">Your ticket has been booked successfully.</p>
        <p style="color:var(--text-muted);font-size:0.85rem;">Booking ID: <strong style="color:var(--primary);">#BMS<?php echo str_pad($booking_id, 6, '0', STR_PAD_LEFT); ?></strong></p>
        ...
        <div class="ticket-card">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;padding-bottom:16px;border-bottom:1px dashed var(--border);">
                ...
                <div>
                    <div style="font-size:1.1rem;font-weight:700;"><?php echo htmlspecialchars($booking['title']); ?></div>
                    <div style="color:var(--text-muted);font-size:0.85rem;"><?php echo htmlspecialchars($booking['language']); ?></div>
                </div>
            </div>

            <?php
            $ticketDetails = [
                '🏟️ Theatre' => $booking['theatre_name'] . ', ' . $booking['city'],
                '📅 Date' => date('D, d M Y', strtotime($booking['show_date'])),
        ...
        <div style="background:var(--bg-dark);border-radius:8px;padding:16px;margin-bottom:24px;text-align:center;">
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=BMS<?php echo str_pad($booking_id, 6, '0', STR_PAD_LEFT); ?>" alt="QR Code" style="margin-bottom:8px; border-radius: 8px;">
        ...
        <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;">
            ...
            <a href="movies.php" class="btn btn-outline">🎬 Book More</a>
        </div>
    </div>
</div>
